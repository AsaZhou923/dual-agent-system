...
当前 readiness 仍为 `policy-v2-compatible-cutover-pending`：Mac 修复已进入 `main` 和本仓库 gitlink，分支/main macOS CI 均成功；生产 Windows 也已读回 `mac_job_contract_version=2` / `mac_permission_profiles_verified=true`。但新 Mac commit 尚未受控部署，真实跨机 policy-v2 canary 也尚未重跑，因此不得把 source/CI 完成表述成 runtime cutover 或 E2E 完成。
